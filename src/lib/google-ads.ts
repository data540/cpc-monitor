...
import { CampaignMetrics, GoogleAdsCampaignRow, CpcDistributionData, GoogleAdsAccount } from '@/types'
...
export async function getCpcDistribution(
  ...
  if (!hourlyRes.ok) {
    ...
    if (!weeklyRes.ok) {
      ...
      console.warn('[cpc-dist] weeklyQuery error:', weeklyRes.status, err.slice(0, 300))
